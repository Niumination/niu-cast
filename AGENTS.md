...
Tanpa ADB, bisa connect via:

1. **USB Tether** — `detect_usb_tether_phone()` cari subnet 192.168.42.x/43.x ✅
2. **WiFi Direct** — mDNS discovery di jaringan lokal ✅
3. **IPv6 Link-Local** — `find_phone_ipv6()` cari MAC prefix di NDP ✅
...
5. ⟳ **v3.3** — UIBC touch/keyboard live test (butuh ADB reconnect)
6. ⟳ **Wireless discovery** — auto-connect via WiFi Direct / USB tether
7. ⟳ **Video streaming** — H.264/H.265 decode
