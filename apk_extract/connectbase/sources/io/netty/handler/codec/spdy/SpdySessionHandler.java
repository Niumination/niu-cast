...
public class SpdySessionHandler extends ChannelDuplexHandler {
    ...
    public SpdySessionHandler(SpdyVersion spdyVersion, boolean z10) {
        this.minorVersion = ((SpdyVersion) ObjectUtil.checkNotNull(spdyVersion, f.f9782f)).getMinorVersion();
        this.server = z10;
    }

    private boolean acceptStream(int i10, byte b10, boolean z10, boolean z11) {
        if (this.receivedGoAwayFrame || this.sentGoAwayFrame) {
            return false;
        }
        boolean zIsRemoteInitiatedId = isRemoteInitiatedId(i10);
        ...
        this.spdySession.acceptStream(i10, b10, z10, z11, this.initialSendWindowSize, this.initialReceiveWindowSize, zIsRemoteInitiatedId);
    ...
    private void halfCloseStream(int i10, boolean z10, ChannelFuture channelFuture) {
        if (z10) {
            this.spdySession.closeRemoteSide(i10, isRemoteInitiatedId(i10));
        } else {
            this.spdySession.closeLocalSide(i10, isRemoteInitiatedId(i10));
        }
    ...
    private void handleOutboundMessage(final ChannelHandlerContext channelHandlerContext, Object obj, ChannelPromise channelPromise) throws Exception {
        if (obj instanceof SpdyDataFrame) {
            ...
            int i10 = spdyDataFrame.content().readableBytes();
            ...
            if (iMin < i10) {
                int i11 = iMin * (-1);
                this.spdySession.updateSendWindowSize(iStreamId, i11);
                this.spdySession.updateSendWindowSize(0, i11);
            ...
            int i12 = i10 * (-1);
            this.spdySession.updateSendWindowSize(iStreamId, i12);
            this.spdySession.updateSendWindowSize(0, i12);
    ...
    private boolean isRemoteInitiatedId(int i10) {
        boolean zIsServerId = SpdyCodecUtil.isServerId(i10);
        boolean z10 = this.server;
        return (z10 && !zIsServerId) || (!z10 && zIsServerId);
    }
    ...
    private void issueStreamError(ChannelHandlerContext channelHandlerContext, int i10, SpdyStreamStatus spdyStreamStatus) {
        boolean zIsRemoteSideClosed = this.spdySession.isRemoteSideClosed(i10);
        ChannelPromise channelPromiseNewPromise = channelHandlerContext.newPromise();
        removeStream(i10, channelPromiseNewPromise);
    ...
    private void removeStream(int i10, ChannelFuture channelFuture) {
        this.spdySession.removeStream(i10, STREAM_CLOSED, isRemoteInitiatedId(i10));
        if (this.closeSessionFutureListener == null || !this.spdySession.noActiveStreams()) {
            return;
        }
        channelFuture.addListener((GenericFutureListener<? extends Future<? super Void>>) this.closeSessionFutureListener);
    ...
    private void updateInitialReceiveWindowSize(int i10) {
        int i11 = i10 - this.initialReceiveWindowSize;
        this.initialReceiveWindowSize = i10;
        this.spdySession.updateAllReceiveWindowSizes(i11);
    }

    private void updateInitialSendWindowSize(int i10) {
        int i11 = i10 - this.initialSendWindowSize;
        this.initialSendWindowSize = i10;
        this.spdySession.updateAllSendWindowSizes(i11);
    }

    private void updateSendWindowSize(final ChannelHandlerContext channelHandlerContext, int i10, int i11) {
        this.spdySession.updateSendWindowSize(i10, i11);
        while (true) {
            SpdySession.PendingWrite pendingWrite = this.spdySession.getPendingWrite(i10);
            if (pendingWrite == null) {
                return;
            }
            SpdyDataFrame spdyDataFrame = pendingWrite.spdyDataFrame;
            int i12 = spdyDataFrame.content().readableBytes();
            int iStreamId = spdyDataFrame.streamId();
            int iMin = Math.min(this.spdySession.getSendWindowSize(iStreamId), this.spdySession.getSendWindowSize(0));
            if (iMin <= 0) {
                return;
            ...
            if (iMin < i12) {
                int i13 = iMin * (-1);
                this.spdySession.updateSendWindowSize(iStreamId, i13);
                this.spdySession.updateSendWindowSize(0, i13);
                channelHandlerContext.writeAndFlush(new DefaultSpdyDataFrame(iStreamId, spdyDataFrame.content().readRetainedSlice(iMin))).addListener(new ChannelFutureListener() { // from class: io.netty.handler.codec.spdy.SpdySessionHandler.3
                    @Override // io.netty.util.concurrent.GenericFutureListener
                    public void operationComplete(ChannelFuture channelFuture) throws Exception {
                        if (channelFuture.isSuccess()) {
                            return;
                ...
                int i14 = i12 * (-1);
                this.spdySession.updateSendWindowSize(iStreamId, i14);
                this.spdySession.updateSendWindowSize(0, i14);
                if (spdyDataFrame.isLast()) {
    ...
    public void channelRead(ChannelHandlerContext channelHandlerContext, Object obj) throws Exception {
        if (obj instanceof SpdyDataFrame) {
            SpdyDataFrame spdyDataFrame = (SpdyDataFrame) obj;
            int iStreamId = spdyDataFrame.streamId();
            int i10 = spdyDataFrame.content().readableBytes() * (-1);
            int iUpdateReceiveWindowSize = this.spdySession.updateReceiveWindowSize(0, i10);
            ...
            if (iUpdateReceiveWindowSize <= this.initialSessionReceiveWindowSize / 2) {
                int i11 = this.initialSessionReceiveWindowSize - iUpdateReceiveWindowSize;
                this.spdySession.updateReceiveWindowSize(0, i11);
            ...
            int iUpdateReceiveWindowSize2 = this.spdySession.updateReceiveWindowSize(iStreamId, i10);
            ...
            if (iUpdateReceiveWindowSize2 <= this.initialReceiveWindowSize / 2 && !spdyDataFrame.isLast()) {
                int i12 = this.initialReceiveWindowSize - iUpdateReceiveWindowSize2;
                this.spdySession.updateReceiveWindowSize(iStreamId, i12);
                channelHandlerContext.writeAndFlush(new DefaultSpdyWindowUpdateFrame(iStreamId, i12));
            }
            if (spdyDataFrame.isLast()) {
    ...
    public void setSessionReceiveWindowSize(int i10) {
        ...
        this.initialSessionReceiveWindowSize = i10;
    }
