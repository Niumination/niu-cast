...
public class SocksCmdResponseDecoder extends ReplayingDecoder<State> {
    ...
    public void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list) throws Exception {
        int i10;
        int i11 = AnonymousClass1.$SwitchMap$io$netty$handler$codec$socks$SocksCmdResponseDecoder$State[state().ordinal()];
        if (i11 == 1) {
        ...
        if (i10 != 1) {
            list.add(new SocksCmdResponse(this.cmdStatus, this.addressType, NetUtil.intToIpAddress(ByteBufUtil.readIntBE(byteBuf)), ByteBufUtil.readUnsignedShortBE(byteBuf)));
        } else if (i10 != 2) {
            list.add(new SocksCmdResponse(this.cmdStatus, this.addressType, SocksCommonUtils.readUsAscii(byteBuf, byteBuf.readByte()), ByteBufUtil.readUnsignedShortBE(byteBuf)));
        } else if (i10 != 3) {
            byte[] bArr = new byte[16];
            byteBuf.readBytes(bArr);
            list.add(new SocksCmdResponse(this.cmdStatus, this.addressType, SocksCommonUtils.ipv6toStr(bArr), ByteBufUtil.readUnsignedShortBE(byteBuf)));
        } else {
            if (i10 == 4) {
                throw new Error();
            }
        ...
        if (i10 != 1) {
            ...
            list.add(new SocksCmdResponse(this.cmdStatus, this.addressType, SocksCommonUtils.ipv6toStr(bArr2), ByteBufUtil.readUnsignedShortBE(byteBuf)));
        } else {
            if (i10 == 4) {
                throw new Error();
            }
