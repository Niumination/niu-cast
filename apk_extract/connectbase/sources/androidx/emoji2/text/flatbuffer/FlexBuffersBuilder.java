...
public class FlexBuffersBuilder {
    ...
    private final ReadWriteBuf f509bb;
    ...
    public static class Value {
        ...
        public static int elemWidth(int i10, int i11, long j10, int i12, int i13) {
            ...
            for (int i14 = 1; i14 <= 32; i14 *= 2) {
                int iWidthUInBits = FlexBuffersBuilder.widthUInBits((int) (((long) ((i13 * i14) + (paddingBytes(i12, i14) + i12))) - j10));
    ...
    private int align(int i10) {
        int i11 = 1 << i10;
        int iPaddingBytes = Value.paddingBytes(this.f509bb.writePosition(), i11);
        while (true) {
            int i12 = iPaddingBytes - 1;
            if (iPaddingBytes == 0) {
                return i11;
            }
            this.f509bb.put((byte) 0);
            iPaddingBytes = i12;
        }
    }
    ...
    private Value createKeyVector(int i10, int i11) {
        long j10 = i11;
        int iMax = Math.max(0, widthUInBits(j10));
        int i12 = i10;
        while (i12 < this.stack.size()) {
            i12++;
            iMax = Math.max(iMax, Value.elemWidth(4, 0, this.stack.get(i12).key, this.f509bb.writePosition(), i12));
        }
        int iAlign = align(iMax);
        writeInt(j10, iAlign);
        int iWritePosition = this.f509bb.writePosition();
    ...
    private Value createVector(int i10, int i11, int i12, boolean z10, boolean z11, Value value) {
        ...
        int iMax = Math.max(0, widthUInBits(j10));
        if (value != null) {
            iMax = Math.max(iMax, value.elemWidth(this.f509bb.writePosition(), 0));
            i13 = 3;
        } else {
            i13 = 1;
        }
        ...
        for (int i16 = i11; i16 < this.stack.size(); i16++) {
            iMax2 = Math.max(iMax2, this.stack.get(i16).elemWidth(this.f509bb.writePosition(), i16 + i13));
            if (z10 && i16 == i11) {
        ...
        if (!z10) {
            while (i17 < this.stack.size()) {
                this.f509bb.put(this.stack.get(i17).storedPackedType(iMax2));
                i17++;
            }
        }
        if (value != null) {
            ...
            typedVector = FlexBuffers.toTypedVector(i15, i14);
        } else {
            typedVector = 10;
        }
        return new Value(i10, typedVector, iMax2, iWritePosition);
    }
    ...
    private int putKey(String str) {
        ...
        if ((this.flags & 1) == 0) {
            ...
            this.f509bb.put(bytes, 0, bytes.length);
            this.f509bb.put((byte) 0);
        ...
        this.f509bb.put(bytes2, 0, bytes2.length);
        this.f509bb.put((byte) 0);
        this.keyPool.put(str, Integer.valueOf(iWritePosition));
        return iWritePosition;
    }
    ...
    private Value writeBlob(int i10, byte[] bArr, int i11, boolean z10) {
        ...
        int iWritePosition = this.f509bb.writePosition();
        this.f509bb.put(bArr, 0, bArr.length);
        if (z10) {
            this.f509bb.put((byte) 0);
        }
        return Value.blob(i10, iWritePosition, i11, iWidthUInBits);
    }

    private void writeDouble(double d10, int i10) {
        if (i10 == 4) {
            this.f509bb.putFloat((float) d10);
        } else if (i10 == 8) {
            this.f509bb.putDouble(d10);
        }
    }

    private void writeInt(long j10, int i10) {
        if (i10 == 1) {
            this.f509bb.put((byte) j10);
            return;
        }
        if (i10 == 2) {
            this.f509bb.putShort((short) j10);
        } else if (i10 == 4) {
            this.f509bb.putInt((int) j10);
        } else {
            if (i10 != 8) {
                return;
            }
            this.f509bb.putLong(j10);
        }
    }

    private void writeOffset(long j10, int i10) {
        writeInt((int) (((long) this.f509bb.writePosition()) - j10), i10);
    }
    ...
    public int endMap(String str, int i10) {
        ...
        Value valueCreateVector = createVector(iPutKey, i10, this.stack.size() - i10, false, false, createKeyVector(i10, this.stack.size() - i10));
    ...
    public int endVector(String str, int i10, boolean z10, boolean z11) {
        Value valueCreateVector = createVector(putKey(str), i10, this.stack.size() - i10, z10, z11, null);
        while (this.stack.size() > i10) {
            ArrayList<Value> arrayList = this.stack;
    ...
    public ByteBuffer finish() {
        int iAlign = align(this.stack.get(0).elemWidth(this.f509bb.writePosition(), 0));
        writeAny(this.stack.get(0), iAlign);
        this.f509bb.put(this.stack.get(0).storedPackedType());
        this.f509bb.put((byte) iAlign);
        this.finished = true;
        return ByteBuffer.wrap(this.f509bb.data(), 0, this.f509bb.writePosition());
    }
    ...
    private void putUInt(String str, long j10) {
        Value valueUInt64;
        int iPutKey = putKey(str);
        int iWidthUInBits = widthUInBits(j10);
        if (iWidthUInBits == 0) {
            valueUInt64 = Value.uInt8(iPutKey, (int) j10);
        } else if (iWidthUInBits == 1) {
            valueUInt64 = Value.uInt16(iPutKey, (int) j10);
        } else if (iWidthUInBits == 2) {
            valueUInt64 = Value.uInt32(iPutKey, (int) j10);
        } else {
            valueUInt64 = Value.uInt64(iPutKey, j10);
        }
    ...
    public void putInt(String str, long j10) {
        int iPutKey = putKey(str);
        if (-128 <= j10 && j10 <= 127) {
            this.stack.add(Value.int8(iPutKey, (int) j10));
            return;
        }
        if (-32768 <= j10 && j10 <= 32767) {
            this.stack.add(Value.int16(iPutKey, (int) j10));
        } else if (-2147483648L <= j10 && j10 <= 2147483647L) {
            this.stack.add(Value.int32(iPutKey, (int) j10));
        } else {
            this.stack.add(Value.int64(iPutKey, j10));
        }
    }
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i10) {
        ...
        this.keyComparator = new Comparator<Value>() { // from class: androidx.emoji2.text.flatbuffer.FlexBuffersBuilder.1
            ...
            public int compare(Value value, Value value2) {
                ...
                int i12 = value2.key;
                do {
                    b10 = FlexBuffersBuilder.this.f509bb.get(i11);
                    b11 = FlexBuffersBuilder.this.f509bb.get(i12);
                    if (b10 == 0) {
                        return b10 - b11;
                    }
                    i11++;
