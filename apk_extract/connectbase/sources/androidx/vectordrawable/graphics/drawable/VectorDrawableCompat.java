...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    private void printGroupTree(VGroup vGroup, int i10) {
        String strA = "";
        for (int i11 = 0; i11 < i10; i11++) {
            strA = a.a(strA, "    ");
        }
        StringBuilder sbA = c.a(strA, "current group is :");
        sbA.append(vGroup.getGroupName());
        sbA.append(" rotation is ");
        sbA.append(vGroup.mRotate);
        Log.v(LOGTAG, sbA.toString());
        Log.v(LOGTAG, strA + "matrix is :" + vGroup.getLocalMatrix().toString());
        for (int i12 = 0; i12 < vGroup.mChildren.size(); i12++) {
            VObject vObject = vGroup.mChildren.get(i12);
            if (vObject instanceof VGroup) {
                printGroupTree((VGroup) vObject, i10 + 1);
            } else {
    ...
    public float getPixelSize() {
        ...
        float f10 = vPathRenderer.mBaseWidth;
        if (f10 == 0.0f) {
            return 1.0f;
        }
        float f11 = vPathRenderer.mBaseHeight;
        if (f11 == 0.0f) {
            return 1.0f;
        }
        float f12 = vPathRenderer.mViewportHeight;
        if (f12 == 0.0f) {
            return 1.0f;
        }
        float f13 = vPathRenderer.mViewportWidth;
        if (f13 == 0.0f) {
            return 1.0f;
        }
    ...
    public static abstract class VPath extends VObject {
        ...
        public String nodesToString(PathParser.PathDataNode[] pathDataNodeArr) {
            ...
            for (int i10 = 0; i10 < pathDataNodeArr.length; i10++) {
                ...
                sbA.append(pathDataNodeArr[i10].mType);
                sbA.append(":");
                string = sbA.toString();
                for (float f10 : pathDataNodeArr[i10].mParams) {
                    StringBuilder sbA2 = k.a.a(string);
                    sbA2.append(f10);
                    sbA2.append(g.f9491d);
        ...
        public void printVPath(int i10) {
            ...
            StringBuilder sbA = c.a(strA, "current path is :");
            sbA.append(this.mPathName);
            sbA.append(" pathData is ");
            sbA.append(nodesToString(this.mNodes));
            Log.v(VectorDrawableCompat.LOGTAG, sbA.toString());
        }
    ...
    public static class VFullPath extends VPath {
        ...
        public void setStrokeColor(int i10) {
            this.mStrokeColor.setColor(i10);
        }
    ...
    public static class VPathRenderer {
        ...
        private void drawGroupTree(VGroup vGroup, Matrix matrix, Canvas canvas, int i10, int i11, ColorFilter colorFilter) {
            ...
            for (int i12 = 0; i12 < vGroup.mChildren.size(); i12++) {
                VObject vObject = vGroup.mChildren.get(i12);
                if (vObject instanceof VGroup) {
                    drawGroupTree((VGroup) vObject, vGroup.mStackedMatrix, canvas, i10, i11, colorFilter);
                } else if (vObject instanceof VPath) {
                    drawPath(vGroup, (VPath) vObject, canvas, i10, i11, colorFilter);
                }
            }
            canvas.restore();
        }

        private void drawPath(VGroup vGroup, VPath vPath, Canvas canvas, int i10, int i11, ColorFilter colorFilter) {
            ...
            if (f12 != 0.0f || vFullPath.mTrimPathEnd != 1.0f) {
                float f13 = vFullPath.mTrimPathOffset;
                float f14 = (f12 + f13) % 1.0f;
                float f15 = (vFullPath.mTrimPathEnd + f13) % 1.0f;
        ...
        public void draw(Canvas canvas, int i10, int i11, ColorFilter colorFilter) {
            drawGroupTree(this.mRootGroup, IDENTITY_MATRIX, canvas, i10, i11, colorFilter);
        }
    ...
    public static class VGroup extends VObject {
        ...
        public boolean onStateChanged(int[] iArr) {
            ...
            for (int i10 = 0; i10 < this.mChildren.size(); i10++) {
                zOnStateChanged |= this.mChildren.get(i10).onStateChanged(iArr);
            }
            return zOnStateChanged;
        }
