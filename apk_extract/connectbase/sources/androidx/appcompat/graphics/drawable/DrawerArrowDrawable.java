...
public class DrawerArrowDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        float fLerp = lerp(this.mBarLength, (float) Math.sqrt(f10 * f10 * 2.0f), this.mProgress);
        ...
        float fLerp4 = lerp(z10 ? 0.0f : -180.0f, z10 ? 180.0f : 0.0f, this.mProgress);
        double d10 = fLerp;
        double d11 = fLerp3;
        boolean z11 = z10;
        ...
        float f11 = (-fLerp2) / 2.0f;
        this.mPath.moveTo(f11 + fRound, 0.0f);
        this.mPath.rLineTo(fLerp2 - (fRound * 2.0f), 0.0f);
        this.mPath.moveTo(f11, fLerp5);
        ...
        float f12 = this.mBarGap;
        canvas.translate(bounds.centerX(), (strokeWidth * 1.5f) + f12 + ((((int) (fHeight - (2.0f * f12))) / 4) * 2));
        if (this.mSpin) {
            canvas.rotate(fLerp4 * (this.mVerticalMirror ^ z11 ? -1 : 1));
        } else if (z11) {
    ...
    public void setBarThickness(float f10) {
        if (this.mPaint.getStrokeWidth() != f10) {
            ...
            this.mMaxCutForBarSize = (float) (Math.cos(ARROW_HEAD_ANGLE) * ((double) (f10 / 2.0f)));
            invalidateSelf();
        }
    }
