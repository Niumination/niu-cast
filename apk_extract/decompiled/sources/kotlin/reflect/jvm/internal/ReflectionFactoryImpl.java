...
public class ReflectionFactoryImpl extends ReflectionFactory {
    ...
    public KType typeOf(KClassifier kClassifier, List<KTypeProjection> list, boolean z2) {
        return kClassifier instanceof ClassBasedDeclarationContainer ? CachesKt.getOrCreateKType(((ClassBasedDeclarationContainer) kClassifier).getJClass(), list, z2) : KClassifiers.createType(kClassifier, list, z2, Collections.emptyList());
    }

    @Override // kotlin.jvm.internal.ReflectionFactory
    public KTypeParameter typeParameter(Object obj, String str, KVariance kVariance, boolean z2) {
        List<KTypeParameter> typeParameters;
        if (obj instanceof KClass) {
            typeParameters = ((KClass) obj).getTypeParameters();
        } else {
            if (!(obj instanceof KCallable)) {
                throw new IllegalArgumentException(a1.a.p(obj, "Type parameter container must be a class or a callable: "));
            }
    ...
    public String renderLambdaToString(FunctionBase functionBase) {
        ...
        return (kFunctionReflect == null || (kFunctionImplAsKFunctionImpl = UtilKt.asKFunctionImpl(kFunctionReflect)) == null) ? super.renderLambdaToString(functionBase) : ReflectionObjectRenderer.INSTANCE.renderLambda(kFunctionImplAsKFunctionImpl.getDescriptor());
    }
}
