...
public final class KFunctionImpl extends KCallableImpl<Object> implements FunctionBase<Object>, KFunction<Object>, FunctionWithAllInvokes {
    ...
    public /* synthetic */ KFunctionImpl(KDeclarationContainerImpl kDeclarationContainerImpl, String str, String str2, FunctionDescriptor functionDescriptor, Object obj, int i8, DefaultConstructorMarker defaultConstructorMarker) {
    ...
    public static final Caller caller_delegate$lambda$4(KFunctionImpl kFunctionImpl) {
        ...
        if (jvmFunctionSignatureMapSignature instanceof JvmFunctionSignature.KotlinConstructor) {
            if (kFunctionImpl.isAnnotationConstructor()) {
                ...
                ArrayList arrayList = new ArrayList(CollectionsKt.i(parameters));
        ...
        if (constructor instanceof Constructor) {
            callerImplCreateJvmStaticInObjectCaller = kFunctionImpl.createConstructorCaller((Constructor) constructor, kFunctionImpl.getDescriptor(), false);
        } else {
            ...
            if (Modifier.isStatic(method.getModifiers())) {
                callerImplCreateJvmStaticInObjectCaller = kFunctionImpl.getDescriptor().getAnnotations().mo1432findAnnotation(UtilKt.getJVM_STATIC()) != null ? kFunctionImpl.createJvmStaticInObjectCaller(method) : kFunctionImpl.createStaticMethodCaller(method);
            } else {
                callerImplCreateJvmStaticInObjectCaller = kFunctionImpl.createInstanceMethodCaller(method);
            }
        }
        return ValueClassAwareCallerKt.createValueClassAwareCallerIfNeeded$default(callerImplCreateJvmStaticInObjectCaller, kFunctionImpl.getDescriptor(), false, 2, null);
    }
    ...
    public static final Caller defaultCaller_delegate$lambda$10(KFunctionImpl kFunctionImpl) {
        ...
        CallerImpl<Constructor<?>> callerImplCreateStaticMethodCaller;
        ...
        if (jvmFunctionSignatureMapSignature instanceof JvmFunctionSignature.KotlinFunction) {
            ...
            if (functionWithDefaultParametersForValueClassOverride != null) {
                ...
                Member memberMo1429getMember = kFunctionImpl.getCaller().mo1429getMember();
                Intrinsics.checkNotNull(memberMo1429getMember);
                genericDeclarationFindDefaultConstructor = container.findDefaultMethod(methodName, methodDesc, !Modifier.isStatic(memberMo1429getMember.getModifiers()));
            }
        ...
        if (genericDeclarationFindDefaultConstructor instanceof Constructor) {
            callerImplCreateStaticMethodCaller = kFunctionImpl.createConstructorCaller((Constructor) genericDeclarationFindDefaultConstructor, kFunctionImpl.getDescriptor(), true);
        } else if (!(genericDeclarationFindDefaultConstructor instanceof Method)) {
            callerImplCreateStaticMethodCaller = null;
        } else if (kFunctionImpl.getDescriptor().getAnnotations().mo1432findAnnotation(UtilKt.getJVM_STATIC()) != null) {
            ...
            if (((ClassDescriptor) containingDeclaration2).isCompanionObject()) {
                callerImplCreateStaticMethodCaller = kFunctionImpl.createStaticMethodCaller((Method) genericDeclarationFindDefaultConstructor);
            } else {
                callerImplCreateStaticMethodCaller = kFunctionImpl.createJvmStaticInObjectCaller((Method) genericDeclarationFindDefaultConstructor);
            }
        } else {
            callerImplCreateStaticMethodCaller = kFunctionImpl.createStaticMethodCaller((Method) genericDeclarationFindDefaultConstructor);
        }
        if (callerImplCreateStaticMethodCaller != null) {
            return ValueClassAwareCallerKt.createValueClassAwareCallerIfNeeded(callerImplCreateStaticMethodCaller, kFunctionImpl.getDescriptor(), true);
        }
        return null;
    }
    ...
    private final FunctionDescriptor getFunctionWithDefaultParametersForValueClassOverride(FunctionDescriptor descriptor) {
        ...
        Member memberMo1429getMember = getCaller().mo1429getMember();
        Intrinsics.checkNotNull(memberMo1429getMember);
        if (!Modifier.isStatic(memberMo1429getMember.getModifiers())) {
            return null;
        }
