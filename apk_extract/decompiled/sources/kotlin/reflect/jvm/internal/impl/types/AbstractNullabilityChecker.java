...
public final class AbstractNullabilityChecker {
    ...
    public final boolean hasNotNullSupertype(TypeCheckerState typeCheckerState, SimpleTypeMarker type, TypeCheckerState.SupertypesPolicy supertypesPolicy) {
        ...
        while (!supertypesDeque.isEmpty()) {
            if (supertypesSet.size() > 1000) {
                StringBuilder sbZ = a.z("Too many supertypes for type: ", type, ". Supertypes = ");
            ...
            if (supertypesSet.add(simpleTypeMarkerPop)) {
                ...
                if (supertypesPolicy2 == null) {
                    ...
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo1441transformType = supertypesPolicy2.mo1441transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(simpleTypeMarkerMo1441transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo1441transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo1441transformType)) {
    ...
    public final boolean hasPathByNotMarkedNullableNodes(TypeCheckerState state, SimpleTypeMarker start, TypeConstructorMarker end) {
        ...
        while (!supertypesDeque.isEmpty()) {
            if (supertypesSet.size() > 1000) {
                StringBuilder sbZ = a.z("Too many supertypes for type: ", start, ". Supertypes = ");
                sbZ.append(CollectionsKt___CollectionsKt.joinToString$default(supertypesSet, null, null, null, 0, null, null, 63, null));
                throw new IllegalStateException(sbZ.toString().toString());
            }
            SimpleTypeMarker simpleTypeMarkerPop = supertypesDeque.pop();
            ...
            if (supertypesSet.add(simpleTypeMarkerPop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(simpleTypeMarkerPop)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo1441transformType = supertypesPolicy.mo1441transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo1441transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo1441transformType);
