...
public final class JvmPackageScope implements MemberScope {
    ...
    private final LazyJavaResolverContext f7034c;
    ...
    public JvmPackageScope(LazyJavaResolverContext c9, JavaPackage jPackage, LazyJavaPackageFragment packageFragment) {
        ...
        this.javaScope = new LazyJavaPackageScope(c9, jPackage, packageFragment);
        this.kotlinScopes$delegate = c9.getStorageManager().createLazyValue(new Function0(this) { // from class: kotlin.reflect.jvm.internal.impl.load.java.lazy.descriptors.JvmPackageScope$$Lambda$0
    ...
    public static final MemberScope[] kotlinScopes_delegate$lambda$1(JvmPackageScope jvmPackageScope) {
        ...
        while (it.hasNext()) {
            MemberScope memberScopeCreateKotlinPackagePartScope = jvmPackageScope.f7034c.getComponents().getDeserializedDescriptorResolver().createKotlinPackagePartScope(jvmPackageScope.packageFragment, (KotlinJvmBinaryClass) it.next());
    ...
    public ClassifierDescriptor mo1439getContributedClassifier(Name name, LookupLocation location) {
        ...
        ClassDescriptor classDescriptorMo1439getContributedClassifier = this.javaScope.mo1439getContributedClassifier(name, location);
        if (classDescriptorMo1439getContributedClassifier != null) {
            return classDescriptorMo1439getContributedClassifier;
        }
        ClassifierDescriptor classifierDescriptor = null;
        for (MemberScope memberScope : getKotlinScopes()) {
            ClassifierDescriptor classifierDescriptorMo1439getContributedClassifier = memberScope.mo1439getContributedClassifier(name, location);
            if (classifierDescriptorMo1439getContributedClassifier != null) {
                if (!(classifierDescriptorMo1439getContributedClassifier instanceof ClassifierDescriptorWithTypeParameters) || !((MemberDescriptor) classifierDescriptorMo1439getContributedClassifier).isExpect()) {
    ...
    public Collection<SimpleFunctionDescriptor> getContributedFunctions(Name name, LookupLocation location) {
        ...
        while (i8 < length) {
            Collection collectionConcat = ScopeUtilsKt.concat(collection, kotlinScopes[i8].getContributedFunctions(name, location));
            i8++;
    ...
    public Collection<PropertyDescriptor> getContributedVariables(Name name, LookupLocation location) {
        ...
        while (i8 < length) {
            Collection collectionConcat = ScopeUtilsKt.concat(collection, kotlinScopes[i8].getContributedVariables(name, location));
            i8++;
            collection = collectionConcat;
        }
        return collection == null ? SetsKt.emptySet() : collection;
    ...
    public void mo1443recordLookup(Name name, LookupLocation location) {
        Intrinsics.checkNotNullParameter(name, "name");
        Intrinsics.checkNotNullParameter(location, "location");
        UtilsKt.record(this.f7034c.getComponents().getLookupTracker(), location, this.packageFragment, name);
    }
