...
public class MemberComparator implements Comparator<DeclarationDescriptor> {
    ...
    public int compare(DeclarationDescriptor declarationDescriptor, DeclarationDescriptor declarationDescriptor2) {
        ...
        if ((declarationDescriptor instanceof TypeAliasDescriptor) && (declarationDescriptor2 instanceof TypeAliasDescriptor)) {
            ...
            for (int i8 = 0; i8 < Math.min(valueParameters.size(), valueParameters2.size()); i8++) {
                DescriptorRenderer descriptorRenderer3 = RENDERER;
                int iCompareTo3 = descriptorRenderer3.renderType(valueParameters.get(i8).getType()).compareTo(descriptorRenderer3.renderType(valueParameters2.get(i8).getType()));
                if (iCompareTo3 != 0) {
                    return iCompareTo3;
                }
            }
            ...
            for (int i9 = 0; i9 < Math.min(typeParameters.size(), typeParameters2.size()); i9++) {
                List<KotlinType> upperBounds = typeParameters.get(i9).getUpperBounds();
                List<KotlinType> upperBounds2 = typeParameters2.get(i9).getUpperBounds();
                int size2 = upperBounds.size() - upperBounds2.size();
                if (size2 != 0) {
                    return size2;
                }
                for (int i10 = 0; i10 < upperBounds.size(); i10++) {
                    DescriptorRenderer descriptorRenderer4 = RENDERER;
                    int iCompareTo4 = descriptorRenderer4.renderType(upperBounds.get(i10)).compareTo(descriptorRenderer4.renderType(upperBounds2.get(i10)));
                    if (iCompareTo4 != 0) {
                        return iCompareTo4;
                    }
                }
