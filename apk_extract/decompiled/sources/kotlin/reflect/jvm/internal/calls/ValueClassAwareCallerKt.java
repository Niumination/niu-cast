...
public final class ValueClassAwareCallerKt {
    ...
    public static final void checkParametersSize(Caller<?> caller, int i8, CallableMemberDescriptor callableMemberDescriptor, boolean z2) {
        if (CallerKt.getArity(caller) == i8) {
            return;
        }
        throw new KotlinReflectionInternalError("Inconsistent number of parameters in the descriptor and Java reflection object: " + CallerKt.getArity(caller) + " != " + i8 + "\nCalling: " + callableMemberDescriptor + "\nParameter types: " + caller.getParameterTypes() + ")\nDefault: " + z2);
    }
    ...
    public static /* synthetic */ Caller createValueClassAwareCallerIfNeeded$default(Caller caller, CallableMemberDescriptor callableMemberDescriptor, boolean z2, int i8, Object obj) {
        if ((i8 & 2) != 0) {
            z2 = false;
        }
    ...
    private static final List<String> getMfvcUnboxMethods$getUnboxMethodNameSuffixes(SimpleType simpleType) {
        ...
        ClassifierDescriptor classifierDescriptorMo1437getDeclarationDescriptor = simpleType.getConstructor().mo1437getDeclarationDescriptor();
        Intrinsics.checkNotNull(classifierDescriptorMo1437getDeclarationDescriptor, "null cannot be cast to non-null type org.jetbrains.kotlin.descriptors.ClassDescriptor");
        MultiFieldValueClassRepresentation<SimpleType> multiFieldValueClassRepresentation = DescriptorUtilsKt.getMultiFieldValueClassRepresentation((ClassDescriptor) classifierDescriptorMo1437getDeclarationDescriptor);
        ...
        for (Pair pair : underlyingPropertyNamesToTypes) {
            ...
            if (mfvcUnboxMethods$getUnboxMethodNameSuffixes != null) {
                List<String> list = mfvcUnboxMethods$getUnboxMethodNameSuffixes;
                ListOf = new ArrayList(CollectionsKt.i(list));
    ...
    public static final Class<?> toInlineClass(KotlinType kotlinType) {
        Class<?> inlineClass = toInlineClass(kotlinType.getConstructor().mo1437getDeclarationDescriptor());
        if (inlineClass == null) {
            return null;
        }
