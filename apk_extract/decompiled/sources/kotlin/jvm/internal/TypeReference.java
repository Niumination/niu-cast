...
public final class TypeReference implements KType {
    ...
    private final String asString(boolean convertPrimitiveToWrapper) {
        ...
        String strL = d.l(name, getArguments().isEmpty() ? "" : CollectionsKt___CollectionsKt.joinToString$default(getArguments(), ", ", "<", ">", 0, null, new d0(this, 6), 24, null), isMarkedNullable() ? "?" : "");
        KType kType = this.platformTypeUpperBound;
        if (!(kType instanceof TypeReference)) {
            return strL;
        }
        String strAsString = ((TypeReference) kType).asString(true);
        if (Intrinsics.areEqual(strAsString, strL)) {
            return strL;
        }
        if (Intrinsics.areEqual(strAsString, strL + '?')) {
            return strL + '!';
        }
        return "(" + strL + ".." + strAsString + ')';
    }
    ...
    public String toString() {
        return a.n(new StringBuilder(), asString(false), " (Kotlin reflection is not available)");
    }
