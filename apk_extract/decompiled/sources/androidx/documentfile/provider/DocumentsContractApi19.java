...
class DocumentsContractApi19 {
    ...
    private static int queryForInt(Context context, Uri uri, String str, int i8) {
        return (int) queryForLong(context, uri, str, i8);
    }

    private static long queryForLong(Context context, Uri uri, String str, long j8) {
        ContentResolver contentResolver = context.getContentResolver();
        Cursor cursorQuery = null;
        try {
            cursorQuery = contentResolver.query(uri, new String[]{str}, null, null, null);
            return (!cursorQuery.moveToFirst() || cursorQuery.isNull(0)) ? j8 : cursorQuery.getLong(0);
        } catch (Exception e) {
            Log.w(TAG, "Failed query: " + e);
            return j8;
        } finally {
