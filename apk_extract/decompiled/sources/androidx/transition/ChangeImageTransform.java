...
public class ChangeImageTransform extends Transition {
    ...
    private static Matrix centerCropMatrix(ImageView imageView) {
        ...
        float fMax = Math.max(width / f, height / f4);
    ...
    private static Matrix copyImageMatrix(@NonNull ImageView imageView) {
        ...
        int i8 = AnonymousClass3.$SwitchMap$android$widget$ImageView$ScaleType[imageView.getScaleType().ordinal()];
        if (i8 != 1) {
            return i8 != 2 ? new Matrix(imageView.getImageMatrix()) : centerCropMatrix(imageView);
        }
