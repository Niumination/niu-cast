        ...
        if (id2 > 0 && (getParent() instanceof MotionLayout)) {
            ...
            if (!this.mAnimateChange) {
                if (!this.mApplyToAllConstraintSets) {
                    changeValue(i9, id2, motionLayout, currentState);
                    return;
                }
                int[] constraintSetIds = motionLayout.getConstraintSetIds();
                while (i12 < constraintSetIds.length) {
                    changeValue(i9, id2, motionLayout, constraintSetIds[i12]);
                    i12++;
                }
                return;
            }
