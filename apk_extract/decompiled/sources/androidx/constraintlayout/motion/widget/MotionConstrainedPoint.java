...
class MotionConstrainedPoint implements Comparable<MotionConstrainedPoint> {
    ...
    private float f1088x;

    /* JADX INFO: renamed from: y, reason: collision with root package name */
    private float f1089y;
    ...
    public void addValues(HashMap<String, ViewSpline> map, int i8) {
        for (String str : map.keySet()) {
            ViewSpline viewSpline = map.get(str);
            str.getClass();
            switch (str) {
                case "rotationX":
                    viewSpline.setPoint(i8, Float.isNaN(this.rotationX) ? 0.0f : this.rotationX);
                    break;
                case "rotationY":
                    viewSpline.setPoint(i8, Float.isNaN(this.rotationY) ? 0.0f : this.rotationY);
                    break;
                case "translationX":
                    viewSpline.setPoint(i8, Float.isNaN(this.translationX) ? 0.0f : this.translationX);
                    break;
                case "translationY":
                    viewSpline.setPoint(i8, Float.isNaN(this.translationY) ? 0.0f : this.translationY);
                    break;
                case "translationZ":
                    viewSpline.setPoint(i8, Float.isNaN(this.translationZ) ? 0.0f : this.translationZ);
                    break;
                case "progress":
                    viewSpline.setPoint(i8, Float.isNaN(this.mProgress) ? 0.0f : this.mProgress);
                    break;
                case "scaleX":
                    viewSpline.setPoint(i8, Float.isNaN(this.scaleX) ? 1.0f : this.scaleX);
                    break;
                case "scaleY":
                    viewSpline.setPoint(i8, Float.isNaN(this.scaleY) ? 1.0f : this.scaleY);
                    break;
                ...
                case "transformPivotY":
                    ...
                    break;
                case "rotation":
                    viewSpline.setPoint(i8, Float.isNaN(this.rotation) ? 0.0f : this.rotation);
                    break;
                case "elevation":
                    viewSpline.setPoint(i8, Float.isNaN(this.elevation) ? 0.0f : this.elevation);
                    break;
                case "transitionPathRotate":
                    viewSpline.setPoint(i8, Float.isNaN(this.mPathRotate) ? 0.0f : this.mPathRotate);
                    break;
                case "alpha":
                    viewSpline.setPoint(i8, Float.isNaN(this.alpha) ? 1.0f : this.alpha);
                    break;
                default:
    ...
    public void fillStandard(double[] dArr, int[] iArr) {
        int i8 = 0;
        float[] fArr = {this.position, this.f1088x, this.f1089y, this.width, this.height, this.alpha, this.elevation, this.rotation, this.rotationX, this.rotationY, this.scaleX, this.scaleY, this.mPivotX, this.mPivotY, this.translationX, this.translationY, this.translationZ, this.mPathRotate};
        for (int i9 : iArr) {
            if (i9 < 18) {
                dArr[i8] = fArr[i9];
                i8++;
    ...
    public void setState(Rect rect, View view, int i8, float f) {
    ...
    public void different(MotionConstrainedPoint motionConstrainedPoint, boolean[] zArr, String[] strArr) {
        ...
        zArr[1] = zArr[1] | diff(this.f1088x, motionConstrainedPoint.f1088x);
        zArr[2] = zArr[2] | diff(this.f1089y, motionConstrainedPoint.f1089y);
