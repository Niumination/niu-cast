...
class RoundRectDrawableWithShadow extends Drawable {
    ...
    public RoundRectDrawableWithShadow(Resources resources, ColorStateList colorStateList, float f, float f4, float f10) {
        ...
        setShadowSize(f4, f10);
    }

    private void buildComponents(Rect rect) {
        float f = this.mRawMaxShadowSize;
        float f4 = SHADOW_MULTIPLIER * f;
        this.mCardBounds.set(rect.left + f, rect.top + f4, rect.right - f, rect.bottom - f4);
        buildShadowCorners();
    }
    ...
    public static float calculateHorizontalPadding(float f, float f4, boolean z2) {
        if (!z2) {
            return f;
        }
        return (float) (((1.0d - COS_45) * ((double) f4)) + ((double) f));
    }

    public static float calculateVerticalPadding(float f, float f4, boolean z2) {
        if (!z2) {
            return f * SHADOW_MULTIPLIER;
        }
        return (float) (((1.0d - COS_45) * ((double) f4)) + ((double) (f * SHADOW_MULTIPLIER)));
    }

    private void drawShadow(Canvas canvas) {
        float f = this.mCornerRadius;
        float f4 = (-f) - this.mShadowSize;
        float f10 = (this.mRawShadowSize / 2.0f) + f + this.mInsetShadow;
        float f11 = f10 * 2.0f;
        boolean z2 = this.mCardBounds.width() - f11 > 0.0f;
        boolean z3 = this.mCardBounds.height() - f11 > 0.0f;
        int iSave = canvas.save();
        RectF rectF = this.mCardBounds;
        canvas.translate(rectF.left + f10, rectF.top + f10);
        canvas.drawPath(this.mCornerShadowPath, this.mCornerShadowPaint);
        if (z2) {
            canvas.drawRect(0.0f, f4, this.mCardBounds.width() - f11, -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        ...
        canvas.drawPath(this.mCornerShadowPath, this.mCornerShadowPaint);
        if (z2) {
            canvas.drawRect(0.0f, f4, this.mCardBounds.width() - f11, (-this.mCornerRadius) + this.mShadowSize, this.mEdgeShadowPaint);
        }
        ...
        canvas.drawPath(this.mCornerShadowPath, this.mCornerShadowPaint);
        if (z3) {
            canvas.drawRect(0.0f, f4, this.mCardBounds.height() - f11, -this.mCornerRadius, this.mEdgeShadowPaint);
        }
    ...
    private void setShadowSize(float f, float f4) {
        if (f < 0.0f) {
