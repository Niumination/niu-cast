...
public abstract class ConstraintController<T> implements ConstraintListener<T> {
    ...
    private void updateCallback(@Nullable OnConstraintUpdatedCallback onConstraintUpdatedCallback, @Nullable T t3) {
    ...
    public abstract boolean isConstrained(@NonNull T t3);
    ...
    public void replace(@NonNull Iterable<WorkSpec> iterable) {
        ...
        for (WorkSpec workSpec : iterable) {
            if (hasConstraint(workSpec)) {
                this.mMatchingWorkSpecIds.add(workSpec.f1155id);
            }
        }
