            ...
            try {
                for (ConstraintController<?> constraintController : this.mConstraintControllers) {
                    if (constraintController.isWorkSpecConstrained(str)) {
                        Logger.get().debug(TAG, "Work " + str + " constrained by " + constraintController.getClass().getSimpleName(), new Throwable[0]);
                        return false;
                    }
                }
                return true;
            ...
            try {
                ...
                for (String str : list) {
                    if (areAllConstraintsMet(str)) {
                        Logger.get().debug(TAG, "Constraints met for " + str, new Throwable[0]);
                        arrayList.add(str);
                    }
                }
