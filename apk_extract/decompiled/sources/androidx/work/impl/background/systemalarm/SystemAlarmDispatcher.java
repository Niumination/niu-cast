...
public class SystemAlarmDispatcher implements ExecutionListener {
            ...
            this.mWorkManager.getWorkTaskExecutor().executeOnBackgroundThread(new Runnable() { // from class: androidx.work.impl.background.systemalarm.SystemAlarmDispatcher.1
                ...
                public void run() {
                    ...
                    if (intent != null) {
                        ...
                        try {
                            Logger.get().debug(str, "Acquiring operation wake lock (" + action + ") " + wakeLockNewWakeLock2, new Throwable[0]);
                            ...
                            Logger.get().debug(str, "Releasing operation wake lock (" + action + ") " + wakeLockNewWakeLock2, new Throwable[0]);
                            ...
                            try {
                                ...
                                Logger.get().debug(str2, "Releasing operation wake lock (" + action + ") " + wakeLockNewWakeLock2, new Throwable[0]);
                                wakeLockNewWakeLock2.release();
                                systemAlarmDispatcher = SystemAlarmDispatcher.this;
                                dequeueAndCheckForCompletion = new DequeueAndCheckForCompletion(systemAlarmDispatcher);
                            } catch (Throwable th3) {
                                Logger.get().debug(SystemAlarmDispatcher.TAG, "Releasing operation wake lock (" + action + ") " + wakeLockNewWakeLock2, new Throwable[0]);
                                wakeLockNewWakeLock2.release();
    ...
    public boolean add(@NonNull Intent intent, int i8) {
        Logger logger = Logger.get();
        String str = TAG;
        logger.debug(str, String.format("Adding command %s (%s)", intent, Integer.valueOf(i8)), new Throwable[0]);
        assertMainThread();
        String action = intent.getAction();
