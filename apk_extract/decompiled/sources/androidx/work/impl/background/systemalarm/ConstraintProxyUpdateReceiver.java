...
public class ConstraintProxyUpdateReceiver extends BroadcastReceiver {
    ...
    public static Intent newConstraintProxyUpdateIntent(Context context, boolean z2, boolean z3, boolean z5, boolean z10) {
        Intent intent = new Intent(ACTION);
        intent.setComponent(new ComponentName(context, (Class<?>) ConstraintProxyUpdateReceiver.class));
        intent.putExtra(KEY_BATTERY_NOT_LOW_PROXY_ENABLED, z2).putExtra(KEY_BATTERY_CHARGING_PROXY_ENABLED, z3).putExtra(KEY_STORAGE_NOT_LOW_PROXY_ENABLED, z5).putExtra(KEY_NETWORK_STATE_PROXY_ENABLED, z10);
        return intent;
    }
    ...
    public void onReceive(@NonNull final Context context, @Nullable final Intent intent) {
        ...
        if (!ACTION.equals(action)) {
            Logger.get().debug(TAG, a.i("Ignoring unknown action ", action), new Throwable[0]);
        } else {
