...
public class Schedulers {
    ...
    public static void schedule(@NonNull Configuration configuration, @NonNull WorkDatabase workDatabase, List<Scheduler> list) {
        ...
        try {
            ...
            if (eligibleWorkForScheduling != null && eligibleWorkForScheduling.size() > 0) {
                ...
                while (it.hasNext()) {
                    workSpecDao.markWorkSpecScheduled(it.next().f1155id, jCurrentTimeMillis);
                }
            }
    ...
    private static Scheduler tryCreateGcmBasedScheduler(@NonNull Context context) {
        try {
            ...
            Logger.get().debug(TAG, "Created androidx.work.impl.background.gcm.GcmScheduler", new Throwable[0]);
            return scheduler;
        } catch (Throwable th2) {
