...
public class AdapterViewBindingAdapter {
    ...
    public static class OnItemSelectedComponentListener implements AdapterView.OnItemSelectedListener {
        ...
        public void onItemSelected(AdapterView<?> adapterView, View view, int i8, long j8) {
            ...
            if (onItemSelected != null) {
                onItemSelected.onItemSelected(adapterView, view, i8, j8);
            }
    ...
    public static void setSelectedItemPosition(AdapterView adapterView, int i8) {
        if (adapterView.getSelectedItemPosition() != i8) {
            adapterView.setSelection(i8);
        }
    }

    @BindingAdapter({"android:selection"})
    public static void setSelection(AdapterView adapterView, int i8) {
        setSelectedItemPosition(adapterView, i8);
    }

    @BindingAdapter({"android:selection", "android:adapter"})
    public static void setSelection(AdapterView adapterView, int i8, Adapter adapter) {
        setSelectedItemPosition(adapterView, i8, adapter);
    }

    @BindingAdapter({"android:selectedItemPosition", "android:adapter"})
    public static void setSelectedItemPosition(AdapterView adapterView, int i8, Adapter adapter) {
        if (adapter != adapterView.getAdapter()) {
            adapterView.setAdapter(adapter);
            adapterView.setSelection(i8);
        } else if (adapterView.getSelectedItemPosition() != i8) {
            adapterView.setSelection(i8);
        }
    }
}
