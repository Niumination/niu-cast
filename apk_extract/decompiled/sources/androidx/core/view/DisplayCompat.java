        ...
        public static ModeCompat[] getSupportedModes(@NonNull Context context, @NonNull Display display) {
            ...
            if (currentDisplaySizeFromWorkarounds == null || physicalSizeEquals(mode, currentDisplaySizeFromWorkarounds)) {
                for (int i8 = 0; i8 < supportedModes.length; i8++) {
                    modeCompatArr[i8] = new ModeCompat(supportedModes[i8], physicalSizeEquals(supportedModes[i8], mode));
                }
            } else {
                for (int i9 = 0; i9 < supportedModes.length; i9++) {
                    modeCompatArr[i9] = physicalSizeEquals(supportedModes[i9], mode) ? new ModeCompat(supportedModes[i9], currentDisplaySizeFromWorkarounds) : new ModeCompat(supportedModes[i9], false);
                }
            }
