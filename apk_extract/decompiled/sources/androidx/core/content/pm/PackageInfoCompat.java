...
public final class PackageInfoCompat {
    ...
    public static class Api28Impl {
        ...
        public static boolean hasSigningCertificate(@NonNull PackageManager packageManager, @NonNull String str, @NonNull byte[] bArr, int i8) {
    ...
    public static boolean hasSignatures(@NonNull PackageManager packageManager, @NonNull String str, @NonNull @Size(min = 1) Map<byte[], Integer> map, boolean z2) throws PackageManager.NameNotFoundException {
        ...
        for (byte[] bArr2 : setKeySet) {
            if (bArr2 == null) {
                throw new IllegalArgumentException(a.i("Cert byte array cannot be null when verifying ", str));
            }
            Integer num = map.get(bArr2);
            if (num == null) {
                throw new IllegalArgumentException(a.i("Type must be specified for cert when verifying ", str));
            }
        ...
        if (signatures.size() != 0 && map.size() <= signatures.size() && (!z2 || map.size() == signatures.size())) {
            if (map.containsValue(1)) {
                ...
                for (int i8 = 0; i8 < signatures.size(); i8++) {
                    bArr[i8] = computeSHA256Digest(signatures.get(i8).toByteArray());
                }
            } else {
                bArr = null;
            }
