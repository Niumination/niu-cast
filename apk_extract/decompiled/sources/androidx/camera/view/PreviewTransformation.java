...
final class PreviewTransformation {
    ...
    private int getRemainingRotationDegrees() {
        int i8 = this.mTargetRotation;
        if (i8 == -1 && !this.mHasCameraTransform) {
            return this.mPreviewRotationDegrees;
        }
        if (this.mHasCameraTransform && i8 != -1) {
            return -CameraOrientationUtil.surfaceRotationToDegrees(i8);
        }
        throw new IllegalStateException("Target rotation must be specified. Target rotation: " + this.mTargetRotation + " hasCameraTransform " + this.mHasCameraTransform);
    }
    ...
    private RectF getTransformedSurfaceRect(Size size, int i8) {
        Preconditions.checkState(isTransformationInfoReady());
        Matrix surfaceToPreviewViewMatrix = getSurfaceToPreviewViewMatrix(size, i8);
    ...
    public Bitmap createTransformedBitmap(@NonNull Bitmap bitmap, Size size, int i8) {
        if (!isTransformationInfoReady()) {
            return bitmap;
        }
        Matrix textureViewCorrectionMatrix = getTextureViewCorrectionMatrix();
        RectF transformedSurfaceRect = getTransformedSurfaceRect(size, i8);
    ...
    @Nullable
    public Matrix getPreviewViewToNormalizedSurfaceMatrix(Size size, int i8) {
        if (!isTransformationInfoReady()) {
            return null;
        }
        Matrix matrix = new Matrix();
        getSurfaceToPreviewViewMatrix(size, i8).invert(matrix);
    ...
    public RectF getPreviewViewViewportRectForMismatchedAspectRatios(Size size, int i8) {
        ...
        return i8 == 1 ? flipHorizontally(rectF2, size.getWidth() / 2.0f) : rectF2;
    }
    ...
    public Matrix getSurfaceToPreviewViewMatrix(Size size, int i8) {
        Preconditions.checkState(isTransformationInfoReady());
        Matrix rectToRect = TransformUtils.getRectToRect(new RectF(this.mSurfaceCropRect), isViewportAspectRatioMatchPreviewView(size) ? new RectF(0.0f, 0.0f, size.getWidth(), size.getHeight()) : getPreviewViewViewportRectForMismatchedAspectRatios(size, i8), this.mPreviewRotationDegrees);
    ...
    public void overrideWithDisplayRotation(int i8, int i9) {
        if (this.mHasCameraTransform) {
            this.mPreviewRotationDegrees = i8;
    ...
    public void setTransformationInfo(@NonNull SurfaceRequest.TransformationInfo transformationInfo, Size size, boolean z2) {
    ...
    public void transformView(Size size, int i8, @NonNull View view) {
        ...
        if (isTransformationInfoReady()) {
            if (view instanceof TextureView) {
                ...
                boolean z2 = false;
                boolean z3 = (!this.mHasCameraTransform || display == null || display.getRotation() == this.mTargetRotation) ? false : true;
