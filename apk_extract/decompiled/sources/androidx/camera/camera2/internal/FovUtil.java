...
public class FovUtil {
    ...
    public static int focalLengthToViewAngleDegrees(float f, float f4) {
        Preconditions.checkArgument(f > 0.0f, "Focal length should be positive.");
        Preconditions.checkArgument(f4 > 0.0f, "Sensor length should be positive.");
        int degrees = (int) Math.toDegrees(Math.atan(f4 / (f * 2.0f)) * 2.0d);
    ...
    public static int getDeviceDefaultViewAngleDegrees(@NonNull CameraManagerCompat cameraManagerCompat, int i8) {
        try {
            for (String str : cameraManagerCompat.getCameraIdList()) {
                ...
                if (num.intValue() == LensFacingUtil.getLensFacingInt(i8)) {
