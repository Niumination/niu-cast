...
public class ExtraSupportedSurfaceCombinationsQuirk implements Quirk {
    ...
    @NonNull
    private List<SurfaceCombination> getLimitedDeviceExtraSupportedFullConfigurations(int i8) {
    ...
    @NonNull
    public List<SurfaceCombination> getExtraSupportedSurfaceCombinations(@NonNull String str, int i8) {
        if (isSamsungS7()) {
            return getSamsungS7ExtraCombinations(str);
        }
        if (supportExtraFullConfigurationsSamsungDevice()) {
            return getLimitedDeviceExtraSupportedFullConfigurations(i8);
        }
        return supportExtraLevel3ConfigurationsGoogleDevice() ? Collections.singletonList(LEVEL_3_LEVEL_PRIV_PRIV_YUV_RAW_CONFIGURATION) : Collections.emptyList();
    }
}
