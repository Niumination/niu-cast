...
public class MaterialMainContainerBackHelper extends MaterialBackAnimationHelper<View> {
    ...
    private ValueAnimator createCornerAnimator(ClippableRoundedCornerLayout clippableRoundedCornerLayout) {
        ...
        valueAnimatorOfFloat.addUpdateListener(new a(clippableRoundedCornerLayout, 3));
    ...
    public void finishBackProgress(long j8, @Nullable View view) {
        AnimatorSet animatorSetCreateResetScaleAndTranslationAnimator = createResetScaleAndTranslationAnimator(view);
        animatorSetCreateResetScaleAndTranslationAnimator.setDuration(j8);
        animatorSetCreateResetScaleAndTranslationAnimator.start();
        resetInitialValues();
    }
    ...
    @VisibleForTesting
    public void updateBackProgress(float f, boolean z2, float f4, float f10) {
        ...
        float f11 = f4 - this.initialTouchY;
        float fLerp3 = AnimationUtils.lerp(0.0f, fMin, Math.abs(f11) / height) * Math.signum(f11);
        ...
        V v3 = this.view;
        if (v3 instanceof ClippableRoundedCornerLayout) {
            ((ClippableRoundedCornerLayout) v3).updateCornerRadius(AnimationUtils.lerp(getExpandedCornerSize(), f10, fInterpolateProgress));
        }
    }
}
