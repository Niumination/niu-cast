...
final class CircularIndeterminateAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private void maybeUpdateSegmentColors(int i8) {
        for (int i9 = 0; i9 < 4; i9++) {
            ...
            if (fractionInRange >= 0.0f && fractionInRange <= 1.0f) {
                ...
                this.activeIndicators.get(0).color = ArgbEvaluatorCompat.getInstance().evaluate(this.interpolator.getInterpolation(fractionInRange), Integer.valueOf(i11), Integer.valueOf(i12)).intValue();
                return;
            }
        }
    }
    ...
    private void updateSegmentPositions(int i8) {
        ...
        for (int i9 = 0; i9 < 4; i9++) {
            activeIndicator.endFraction = (this.interpolator.getInterpolation(getFractionInRange(i8, DELAY_TO_EXPAND_IN_MS[i9], 667)) * 250.0f) + activeIndicator.endFraction;
            activeIndicator.startFraction = (this.interpolator.getInterpolation(getFractionInRange(i8, DELAY_TO_COLLAPSE_IN_MS[i9], 667)) * 250.0f) + activeIndicator.startFraction;
        }
        float f4 = activeIndicator.startFraction;
        float f10 = activeIndicator.endFraction;
        activeIndicator.startFraction = (((f10 - f4) * this.completeEndFraction) + f4) / 360.0f;
