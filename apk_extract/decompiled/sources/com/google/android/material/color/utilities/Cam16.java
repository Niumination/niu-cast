...
public final class Cam16 {
    ...
    private Cam16(double d7, double d10, double d11, double d12, double d13, double d14, double d15, double d16, double d17) {
        this.hue = d7;
        this.chroma = d10;
        this.f2217j = d11;
        this.q = d12;
    ...
    public static Cam16 fromIntInViewingConditions(int i8, ViewingConditions viewingConditions) {
        double dLinearized = ColorUtils.linearized((16711680 & i8) >> 16);
        double dLinearized2 = ColorUtils.linearized((65280 & i8) >> 8);
        double dLinearized3 = ColorUtils.linearized(i8 & 255);
    ...
    public static Cam16 fromJch(double d7, double d10, double d11) {
        return fromJchInViewingConditions(d7, d10, d11, ViewingConditions.DEFAULT);
    }

    private static Cam16 fromJchInViewingConditions(double d7, double d10, double d11, ViewingConditions viewingConditions) {
        ...
        double d13 = (1.7000000000000002d * d7) / ((0.007d * d7) + 1.0d);
        double dLog1p = 43.859649122807014d * Math.log1p(flRoot * 0.0228d);
        return new Cam16(d11, d10, d7, aw, flRoot, dSqrt, d13, Math.cos(radians) * dLog1p, Math.sin(radians) * dLog1p);
    }

    public static Cam16 fromUcs(double d7, double d10, double d11) {
        return fromUcsInViewingConditions(d7, d10, d11, ViewingConditions.DEFAULT);
    }

    public static Cam16 fromUcsInViewingConditions(double d7, double d10, double d11, ViewingConditions viewingConditions) {
        ...
        return fromJchInViewingConditions(d7 / (1.0d - ((d7 - 100.0d) * 0.007d)), dExpm1, dAtan2, viewingConditions);
    }

    public static Cam16 fromXyzInViewingConditions(double d7, double d10, double d11, ViewingConditions viewingConditions) {
        double[][] dArr = XYZ_TO_CAM16RGB;
        double[] dArr2 = dArr[0];
        double d12 = (dArr2[2] * d11) + (dArr2[1] * d10) + (dArr2[0] * d7);
        double[] dArr3 = dArr[1];
        double d13 = (dArr3[2] * d11) + (dArr3[1] * d10) + (dArr3[0] * d7);
        double[] dArr4 = dArr[2];
        double d14 = (dArr4[2] * d11) + (dArr4[1] * d10) + (dArr4[0] * d7);
    ...
    public double[] xyzInViewingConditions(ViewingConditions viewingConditions, double[] dArr) {
        ...
        double d7 = (((0.305d + nbb) * 23.0d) * dPow) / (((dPow * 108.0d) * dSin) + (((11.0d * dPow) * dCos2) + (ncb * 23.0d)));
        double d10 = dCos2 * d7;
        double d11 = d7 * dSin;
