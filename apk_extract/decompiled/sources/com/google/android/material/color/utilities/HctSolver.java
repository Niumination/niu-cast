...
public class HctSolver {
    ...
    public static boolean areInCyclicOrder(double d7, double d10, double d11) {
        return sanitizeRadians(d10 - d7) < sanitizeRadians(d11 - d7);
    }

    public static double[] bisectToLimit(double d7, double d10) {
        int iCriticalPlaneAbove;
        int iCriticalPlaneBelow;
        double[][] dArrBisectToSegment = bisectToSegment(d7, d10);
        ...
        for (int i8 = 0; i8 < 3; i8++) {
            ...
            if (d11 != d12) {
                if (d11 < d12) {
                    ...
                    iCriticalPlaneBelow = criticalPlaneAbove(trueDelinearized(dArr2[i8]));
                } else {
                    iCriticalPlaneAbove = criticalPlaneAbove(trueDelinearized(d11));
                    iCriticalPlaneBelow = criticalPlaneBelow(trueDelinearized(dArr2[i8]));
                }
                int i9 = iCriticalPlaneAbove;
                int i10 = iCriticalPlaneBelow;
                for (int i11 = 0; i11 < 8 && Math.abs(i10 - i9) > 1; i11++) {
                    int iFloor = (int) Math.floor(((double) (i9 + i10)) / 2.0d);
                    double[] coordinate = setCoordinate(dArr, CRITICAL_PLANES[iFloor], dArr2, i8);
    ...
    public static double[][] bisectToSegment(double d7, double d10) {
        ...
        for (int i8 = 0; i8 < 12; i8++) {
            double[] dArrNthVertex = nthVertex(d7, i8);
            if (dArrNthVertex[0] >= 0.0d) {
                ...
                if (!z2) {
                    ...
                    d11 = dHueOf;
                    d12 = d11;
                } else if (z3 || areInCyclicOrder(d11, dHueOf, d12)) {
    ...
    public static double chromaticAdaptation(double d7) {
        double dPow = Math.pow(Math.abs(d7), 0.42d);
        return ((((double) MathUtils.signum(d7)) * 400.0d) * dPow) / (dPow + 27.13d);
    }
    ...
    public static int findResultByJ(double d7, double d10, double d11) {
        double dSqrt = Math.sqrt(d11) * 11.0d;
        ViewingConditions viewingConditions = ViewingConditions.DEFAULT;
        double d12 = 1.0d;
        double dPow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d);
        double ncb = viewingConditions.getNcb() * viewingConditions.getNc() * (Math.cos(d7 + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d;
    ...
    public static double intercept(double d7, double d10, double d11) {
        return (d10 - d7) / (d11 - d7);
    }
    ...
    public static double inverseChromaticAdaptation(double d7) {
        ...
        return Math.pow(dMax, 2.380952380952381d) * ((double) MathUtils.signum(d7));
    }

    public static boolean isBounded(double d7) {
        return 0.0d <= d7 && d7 <= 100.0d;
    }

    public static double[] lerpPoint(double[] dArr, double d7, double[] dArr2) {
        double d10 = dArr[0];
        double d11 = ((dArr2[0] - d10) * d7) + d10;
        double d12 = dArr[1];
        double d13 = ((dArr2[1] - d12) * d7) + d12;
        double d14 = dArr[2];
        return new double[]{d11, d13, ((dArr2[2] - d14) * d7) + d14};
    }
    ...
    public static double[] nthVertex(double d7, int i8) {
        double[] dArr = Y_FROM_LINRGB;
        double d10 = dArr[0];
        double d11 = dArr[1];
        double d12 = dArr[2];
        double d13 = i8 % 4 <= 1 ? 0.0d : 100.0d;
        double d14 = i8 % 2 == 0 ? 0.0d : 100.0d;
        if (i8 < 4) {
            double d15 = ((d7 - (d11 * d13)) - (d12 * d14)) / d10;
            return isBounded(d15) ? new double[]{d15, d13, d14} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        if (i8 < 8) {
            double d16 = ((d7 - (d10 * d14)) - (d12 * d13)) / d11;
            return isBounded(d16) ? new double[]{d14, d16, d13} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d17 = ((d7 - (d10 * d13)) - (d11 * d14)) / d12;
    ...
    public static double[] setCoordinate(double[] dArr, double d7, double[] dArr2, int i8) {
        return lerpPoint(dArr, intercept(dArr[i8], d7, dArr2[i8]), dArr2);
    }

    public static Cam16 solveToCam(double d7, double d10, double d11) {
        return Cam16.fromInt(solveToInt(d7, d10, d11));
    }

    public static int solveToInt(double d7, double d10, double d11) {
        if (d10 < 1.0E-4d || d11 < 1.0E-4d || d11 > 99.9999d) {
            return ColorUtils.argbFromLstar(d11);
        }
        double dSanitizeDegreesDouble = (MathUtils.sanitizeDegreesDouble(d7) / 180.0d) * 3.141592653589793d;
