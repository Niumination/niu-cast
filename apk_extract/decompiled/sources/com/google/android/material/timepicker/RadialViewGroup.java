...
class RadialViewGroup extends ConstraintLayout {
    ...
    private void addConstraints(List<View> list, ConstraintSet constraintSet, int i8) {
        ...
        float size = 0.0f;
        while (it.hasNext()) {
            constraintSet.constrainCircle(it.next().getId(), R.id.circle_center, i8, size);
    ...
    @Dimension
    public int getLeveledRadius(int i8) {
        int i9 = this.radius;
        return i8 == 2 ? Math.round(i9 * LEVEL_RADIUS_RATIO) : i9;
    }
        ...
        for (int i8 = 0; i8 < getChildCount(); i8++) {
            ...
            if (childAt.getId() != R.id.circle_center && !shouldSkipView(childAt)) {
                ...
                if (!map.containsKey(i9)) {
                    map.put(i9, new ArrayList());
                }
                ((List) map.get(i9)).add(childAt);
            }
        }
    ...
    public RadialViewGroup(@NonNull Context context, @Nullable AttributeSet attributeSet, int i8) {
        ...
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, R.styleable.RadialViewGroup, i8, 0);
        ...
        this.updateLayoutParametersRunnable = new Runnable() { // from class: com.google.android.material.timepicker.b
            ...
            public final void run() {
                this.f2271a.updateLayoutParams();
            }
        };
