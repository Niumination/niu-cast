...
public abstract class AbstractMessage extends AbstractMessageLite implements Message {
    ...
    public static abstract class Builder<BuilderType extends Builder<BuilderType>> extends AbstractMessageLite.Builder implements Message.Builder {
        ...
        public BuilderType mergeFrom(byte[] bArr, int i8, int i9, ExtensionRegistryLite extensionRegistryLite) throws InvalidProtocolBufferException {
            return (BuilderType) super.mergeFrom(bArr, i8, i9, extensionRegistryLite);
        }
            ...
            if (fieldDescriptor.getType() == Descriptors.FieldDescriptor.Type.BYTES) {
                if (fieldDescriptor.isRepeated()) {
                    ...
                    for (int i8 = 0; i8 < list.size(); i8++) {
                        if (!compareBytes(list.get(i8), list2.get(i8))) {
                            return false;
                        }
                    }
    ...
    public static int hashFields(int i8, Map<Descriptors.FieldDescriptor, Object> map) {
        int i9;
        int iHashEnum;
        for (Map.Entry<Descriptors.FieldDescriptor, Object> entry : map.entrySet()) {
            Descriptors.FieldDescriptor key = entry.getKey();
            Object value = entry.getValue();
            int number = key.getNumber() + (i8 * 37);
            if (key.isMapField()) {
                i8 = (number * 53) + hashMapField(value);
            } else {
