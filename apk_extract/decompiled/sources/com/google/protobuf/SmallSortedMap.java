...
class SmallSortedMap<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public class DescendingEntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        public boolean hasNext() {
            int i8 = this.pos;
            return (i8 > 0 && i8 <= SmallSortedMap.this.entryList.size()) || getOverflowIterator().hasNext();
        }
    ...
    public class Entry implements Map.Entry<K, V>, Comparable<SmallSortedMap<K, V>.Entry> {
        ...
        public int hashCode() {
            K k8 = this.key;
            int iHashCode = k8 == null ? 0 : k8.hashCode();
            V v3 = this.value;
    ...
    public class EntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        public void remove() {
            ...
            int i8 = this.pos;
            this.pos = i8 - 1;
            smallSortedMap.removeArrayEntryAt(i8);
        }
        ...
        public Map.Entry<K, V> next() {
            ...
            int i8 = this.pos + 1;
            this.pos = i8;
            return i8 < SmallSortedMap.this.entryList.size() ? (Map.Entry) SmallSortedMap.this.entryList.get(this.pos) : getOverflowIterator().next();
        }
    }
    ...
    private int binarySearchInArray(K k8) {
        ...
        if (i11 < 0) {
            ...
            while (i8 <= i11) {
                i10 = (i8 + i11) / 2;
                iCompareTo = k8.compareTo(this.entryList.get(i10).getKey());
                if (iCompareTo < 0) {
                    i11 = i10 - 1;
                } else {
                    if (iCompareTo > 0) {
            ...
            int iCompareTo2 = k8.compareTo(this.entryList.get(i11).getKey());
            if (iCompareTo2 > 0) {
                i9 = size + 1;
            } else {
                if (iCompareTo2 == 0) {
    ...
    public static <FieldDescriptorType extends FieldSet.FieldDescriptorLite<FieldDescriptorType>> SmallSortedMap<FieldDescriptorType, Object> newFieldMap(int i8) {
            ...
            public void makeImmutable() {
                if (!isImmutable()) {
                    for (int i9 = 0; i9 < getNumArrayEntries(); i9++) {
                        Map.Entry<FieldDescriptorType, Object> arrayEntryAt = getArrayEntryAt(i9);
    ...
    public static <K extends Comparable<K>, V> SmallSortedMap<K, V> newInstanceForTest(int i8) {
        return new SmallSortedMap<>(i8);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public V removeArrayEntryAt(int i8) {
        checkMutable();
        V value = this.entryList.remove(i8).getValue();
    ...
    public boolean equals(Object obj) {
        ...
        for (int i8 = 0; i8 < numArrayEntries; i8++) {
            if (!getArrayEntryAt(i8).equals(smallSortedMap.getArrayEntryAt(i8))) {
                return false;
            }
        }
    ...
    public Map.Entry<K, V> getArrayEntryAt(int i8) {
        return this.entryList.get(i8);
    }
    ...
    public int hashCode() {
        ...
        int iHashCode = 0;
        for (int i8 = 0; i8 < numArrayEntries; i8++) {
            iHashCode += this.entryList.get(i8).hashCode();
        }
    ...
    public V put(K k8, V v3) {
        checkMutable();
        int iBinarySearchInArray = binarySearchInArray(k8);
        if (iBinarySearchInArray >= 0) {
            return this.entryList.get(iBinarySearchInArray).setValue(v3);
        }
        ensureEntryArrayMutable();
        int i8 = -(iBinarySearchInArray + 1);
        if (i8 >= this.maxArraySize) {
            return getOverflowEntriesMutable().put(k8, v3);
        }
        int size = this.entryList.size();
        int i9 = this.maxArraySize;
        if (size == i9) {
            SmallSortedMap<K, V>.Entry entryRemove = this.entryList.remove(i9 - 1);
            getOverflowEntriesMutable().put(entryRemove.getKey(), entryRemove.getValue());
        }
        this.entryList.add(i8, new Entry(k8, v3));
        return null;
    }
}
