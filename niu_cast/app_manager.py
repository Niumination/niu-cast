...
NOTE: PyQt5 imports are lazy to avoid CLI dependency.
"""
...
class AppManagerWidget:
    """Proxy that lazily creates the real QWidget on first use."""
    ...
    def __getattr__(self, name):
        """Delegate all attribute access to the real widget once created."""
        if self._widget is None:
            self._widget = self._build()
        return getattr(self._widget, name)

    def _build(self):
        """Lazily import PyQt5 and build the real widget."""
        ...
        class _RealAppManager(QWidget):
            ...
            def _setup_ui(self):
                ...
                self.lbl_count.setStyleSheet(
                    f"color: {PALETTE['dim']}; font-size: 10px; padding-left: 2px;"
                )
        ...
        widget = _RealAppManager(self._adb)
